...
export class ListingService {
  constructor(private firestore: Firestore, private auth: Auth, private bookingService: BookingService) {}
  ...
  getAllListings(): Observable<Car[]> {
    ...
    return this.bookingService.getAllBookedCarIds().pipe(
      // Use the booking IDs to filter out listings
      switchMap((bookedCarIds) =>
        from(
          getDocs(listingsQuery).then((snapshot) =>
            snapshot.docs
              .map((doc) => ({
              ...
              .filter((listing) => !bookedCarIds.includes(listing.id)) // Exclude booked listings
          )
        )
      ),
